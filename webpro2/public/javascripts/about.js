...
//       document.getElementById("host_name").innerHTML = x[0];
//       document.getElementById("host_say").innerHTML = x[1];
//     }
//   }
//   xmlhttp.open("GET", "http://localhost/house1.php", true);
//   xmlhttp.send();
// })

window.requestAnimationFrame = (function(){
  return window.requestAnimationFrame || //告诉浏览器你希望执行一个动画，并且要求浏览器在下次重绘之前调用指定的回调函数更新动画
         window.webkitRequestAnimationFrame ||  //告诉浏览器你想在浏览器的下个重绘（repaint）之前来执行一个动画或者执行浏览器通过调用一个特定的函数来更新动画的请求
         window.mozRequestAnimationFrame || //告诉浏览器你希望执行一个动画，并且要求浏览器在下次重绘之前调用指定的回调函数更新动画。
         function( callback ){
              window.setTimeout( callback, 1000/2 );
         };
})();
var myCanvas = document.getElementById("canvas");
var ctx = myCanvas.getContext("2d");//getContext 设置画笔 画2d的
var num;
var w,h;
var duixiang = [];
var move = {};
function widthheight(){
  ...
  h = myCanvas.height = window.innerHeight;  //显示区的宽和高
